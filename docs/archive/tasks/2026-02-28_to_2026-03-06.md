...
### T-076 — A1先行実装: sync-v2 contract model + cursor session + XPC plumbing (2026-03-06)
- **Status**: DONE
- **Priority**: P0
- **Description**:
  - daemon A1 完了前に、term 側で `ui.bootstrap.v2` / `ui.changes.v2` の decode model と cursor session を先行実装する。
  - cursor ownership は XPC service 側に寄せ、app process が raw cursor を保持しなくてよい形を先に作る。
  - AppViewModel はまだ A0 snapshot path のまま維持し、daemon wire contract 固定後に切替える。
- **Implemented**:
  - `AgtmuxSyncV2Models` を追加:
    - bootstrap / changes / resync_required / cursor 型を追加
    - snake_case decode と ISO8601 date decode 前提の contract を固定
  - `AgtmuxSyncV2Session` を追加:
    - bootstrap 必須
    - success 時に `nextCursor` を前進
    - `resync_required` で cursor を明示クリア
  - `AgtmuxDaemonClient` に sync-v2 transport を追加:
    - local daemon socket への JSON-RPC 呼び出しを追加
    - test 用 env override を追加
  - XPC plumbing を追加:
    - `fetchUIBootstrapV2`
    - `fetchUIChangesV2(limit:)`
    - `resetUIChangesV2`
    - XPC service 側で `AgtmuxSyncV2Session` を保持
  - `project.yml` ベースで `xcodegen generate` を再実行し、new files を project へ反映
- **Files**:
...
- **Acceptance Criteria**:
  - [x] sync-v2 model decode が core test で固定される
  - [x] cursor session の bootstrap / advance / resync-clear が core test で固定される
  - [x] `swift build` が通る
  - [x] `xcodebuild -project AgtmuxTerm.xcodeproj -scheme AgtmuxTerm -destination 'platform=macOS' build` が通る
  - [x] AppViewModel の現行 A0 path を壊さない

### T-077 — A1実装: AppViewModel local metadata consumer を sync-v2 へ切替 (2026-03-06)
- **Status**: DONE
- **Priority**: P0
- **Description**:
  - daemon 側 `ui.bootstrap.v2` / `ui.changes.v2` 実装完了を受けて、term 側 consumer を A0 snapshot path から sync-v2 へ切替える。
  - inventory-first / non-destructive overlay の UX 契約は維持しつつ、local metadata overlay を bootstrap + ordered changes で更新する。
  - cursor ownership は app process に持ち込まず、既存の cursorless local client API を使う。
- **Implemented**:
  - `LocalMetadataClient` protocol を追加し、cursorless local API を term 側の正式 contract として固定。
    - `fetchUIBootstrapV2()`
    - `fetchUIChangesV2(limit:)`
    - `resetUIChangesV2()`
  - `AgtmuxDaemonClient` に in-process sync-v2 session ownership を追加。
    - direct daemon path でも app process は raw cursor を保持しない
  - daemon actual payload shape に合わせて sync-v2 model を補正。
    - `resync_required` nested object decode
    - change entry の `pane` / `session` snapshot decode
    - bootstrap pane decode は `session_name` / `window_id` 欠落を許容
  - `AppViewModel` の local metadata refresh を sync-v2 consumer へ切替。
    - 初回 bootstrap で overlay cache を構築
    - 以降は ordered `ui.changes.v2` で overlay cache を前進
    - `resync_required` は explicit reset + bootstrap として処理
    - transport error 時も cached overlay は保持
  - integration test を拡張し、A0 UX 契約を sync-v2 path で再固定。
- **Files**:
  ...
  - `Tests/AgtmuxTermIntegrationTests/AppViewModelA0Tests.swift`
- **Acceptance Criteria**:
  - [x] term 側が local metadata 更新に `ui.bootstrap.v2` / `ui.changes.v2` を使う
  - [x] inventory-first / non-destructive overlay 契約が integration test で維持される
  - [x] `resync_required` が explicit resync として処理される
  - [x] `swift test -q --filter AgtmuxSyncV2DecodingTests` が通る
  - [x] `swift test -q --filter AgtmuxSyncV2SessionTests` が通る
  - [x] `swift test -q --filter AppViewModelA0Tests` が通る
  - [x] `swift build` が通る
  - [x] `xcodebuild -project AgtmuxTerm.xcodeproj -scheme AgtmuxTerm -destination 'platform=macOS' build` が通る

### T-078 — A1 hardening: bundled release daemon + source build override の二層構成へ切替 (2026-03-06)
- **Status**: DONE
- **Priority**: P0
- **Description**:
  - `swift run AgtmuxTerm` / packaged app が global `/tmp/agtmux-$USER/agtmuxd.sock` 上の古い Homebrew daemon を再利用し、`ui.bootstrap.v2` が `method not found` になる構成事故を排除する。
  - packaged app は bundle 同梱の release daemon を app-owned socket で使い、development は `AGTMUX_BIN` explicit override で source build を使う二層構成へ切替える。
  - PATH fallback は term の local daemon resolution から外し、protocol mismatch は fail-loudly に surfacing する。
- **Implemented**:
  - runtime policy を term app / XPC service / direct client に固定:
    - app-owned socket: `~/Library/Application Support/AGTMUXDesktop/agtmuxd.sock`
    - binary source: `AGTMUX_BIN` explicit override or bundled `Resources/Tools/agtmux`
    - PATH / common-location fallback 廃止
  - `AgtmuxBinaryResolver` を hardening し、socket parent directory 作成 helper を追加。
  - `AgtmuxDaemonSupervisor` / `AgtmuxDaemonService` の daemon launch 前に app-owned socket directory を明示作成。
  - default runtime に bundled daemon / `AGTMUX_BIN` が存在しない場合は fail-closed に `daemon unavailable` を surfacing。
  - sync-v2 old-daemon (`-32601` / `method not found`) を structured UI error として分類。
    - direct client: `DaemonError.processError(stderr:)`
    - XPC path: existing `NSString?` error slot に同じ structured payload を通す
  - `AppViewModel` / `SidebarView` に local daemon incompatibility state を追加し、inventory-first を維持したまま banner / empty-state で surfacing。
  - follow-up fix:
    - `localDaemonUnavailable` issue を追加
    - stale `localDaemonIssue` を later non-classified failure / offline transition で clear
  - focused runtime hardening test を追加し、`AGTMUX_BIN` 優先 / no PATH fallback / app-owned socket / missing runtime fail-closed / structured sync-v2 incompatibility surface を固定。
- **Files**:
  ...
  - `docs/90_index.md`
- **Acceptance Criteria**:
  - [x] packaged app の local daemon は bundle binary + app-owned socket を使う
  - [x] `swift run AgtmuxTerm` は `AGTMUX_BIN` 未設定時に PATH 上 daemon を暗黙採用しない
  - [x] global `/tmp/agtmux-$USER/agtmuxd.sock` の old daemon と term app の local metadata path が分離される
  - [x] default runtime に bundled daemon / `AGTMUX_BIN` が無い場合は `daemon unavailable` が明示 surfacing される
  - [x] incompatible daemon (`ui.bootstrap.v2` missing) が明示 surfacing される
  - [x] `swift test -q --filter RuntimeHardeningTests` が通る
  - [x] `swift test -q --filter AppViewModelA0Tests` が通る
  - [x] `swift test -q --filter AppViewModelLiveManagedAgentTests` が通る
  - [x] `swift build` が通る
  - [x] `xcodebuild -project AgtmuxTerm.xcodeproj -scheme AgtmuxTerm -destination 'platform=macOS' build` が通る

### T-079 — A2 kickoff: daemon observability contract 固定 + cross-repo handover 作成 (2026-03-06)
- **Status**: DONE
- **Priority**: P0
- **Description**:
  - A1 closeout 完了を受けて、A2 を term/daemon 並列実装できるように design-tier docs と handover を固定する。
  - daemon 側は replay/ack compaction と `ui.health.v1` observability payload を担当し、term 側は health consumer / sidebar surfacing を担当する。
- **Implemented**:
  - `FR-016` / `FR-017` として A2 observability requirement を spec に追加。
  - architecture/design に `ui.health.v1` data flow と payload contract を追加。
  - daemon 並列開発向け handover を `/tmp/agtmux-v2-a2-cross-repo-handover-20260306.md` に作成。
  - daemon 側 A2 4 項目（replay/ack compaction、additive `ui.health.v1`、実測 health、`ui.bootstrap.v2` / `ui.changes.v2` 互換維持）の実装完了を agtmux repo の `T-XTERM-A2` / progress で確認。
- **Planned**:
  - term 側:
    - `AgtmuxUIHealthV1` models
    - `LocalHealthClient`
    - `AppViewModel` health state
    - sidebar health strip / badge / tooltip
  - daemon 側:
    - replay/ack compaction
    - `ui.health.v1`
    - health counters / timestamps / resync reasons
- **Files**:
  - `docs/20_spec.md`
  - `docs/30_architecture.md`
  - `docs/40_design.md`
...
- **Acceptance Criteria**:
  - [x] A2 の term/daemon split が docs に記録されている
  - [x] `ui.health.v1` payload contract が docs に記録されている
  - [x] daemon 並列開発向け handover が `/tmp` に存在する
  - [x] term 側 `ui.health.v1` consumer 実装が開始される
  - [x] daemon 側 A2 実装が handover 契約に沿って着手される

### T-080 — A2 term implementation: `ui.health.v1` consumer + sidebar surfacing (2026-03-06)
- **Status**: DONE
- **Priority**: P0
- **Description**:
  - daemon 側 A2 実装と並列で、term 側の health consumer と UI surfacing を先行実装する。
  - `ui.health.v1` は additive observability path として扱い、inventory-first / sync-v2 metadata overlay 契約は壊さない。
- **Planned**:
  - core:
    - `AgtmuxUIHealthV1` models を追加
    - `AgtmuxDaemonClient` direct path に `ui.health.v1` RPC を追加
    - XPC contract / XPC client / bundled service に health fetch path を追加
    - test 用 env override を追加
  - app:
    - `LocalHealthClient` contract を追加
    - `AppViewModel` に local daemon health state と poll loop を追加
    - health fetch failure は non-destructive に扱い、pane existence / metadata overlay は維持
  - UI:
    - sidebar に compact health strip / badge / tooltip を追加
    - `runtime` / `replay` / `overlay` / `focus` の degraded/unavailable を短く surfacing
  - tests:
    - decode / client tests
    - `AppViewModel` integration tests for health available / degraded / unavailable
- **Files**:
  ...
  - `Sources/AgtmuxTerm/AppViewModel.swift`
  - `Sources/AgtmuxTerm/SidebarView.swift`
  - `Tests/AgtmuxTermCoreTests/*`
  - `Tests/AgtmuxTermIntegrationTests/*`
- **Acceptance Criteria**:
  - [x] term 側に `ui.health.v1` typed models が存在する
  - [x] direct path / XPC path の両方で `ui.health.v1` を取得できる
  - [x] `AppViewModel` が health state を保持し、pane existence と独立に更新できる
  - [x] sidebar に health surfacing が追加される
  - [x] `swift test -q --filter RuntimeHardeningTests` が通る
  - [x] `swift test -q --filter AgtmuxSyncV2DecodingTests` が通る
  - [x] `swift test -q --filter AppViewModelA0Tests` が通る
  - [x] health 関連の focused tests が通る

### T-081 — A2 follow-up: sidebar health surfacing の UI coverage / accessibility 固定 (2026-03-06)
- **Status**: DONE
- **Priority**: P1
- **Description**:
  - `T-080` で `ui.health.v1` consumer と sidebar surfacing は実装済みだが、health strip 自体の dedicated UI coverage と accessibility hooks は未固定。
  - A2 closeout 前に health strip の visible contract を UI test か lightweight view test で固定し、degraded / unavailable surfacing の regression を拾えるようにする。
- **Planned**:
  - sidebar health strip に stable accessibility identifiers / labels を付与する。
  - inventory-only UI test path と衝突しない metadata-enabled test path を決める。
  - degraded / unavailable / cleared state の 3 パターンを coverage する。
- **Implemented**:
  - `sidebar.health.strip` / `sidebar.health.badge.*` accessibility contract を追加。
  - `launchForMetadataUITest()` を追加し、inventory-only 既定を維持したまま metadata-enabled UI path を分離した。
  - targeted XCUITest 2 本 (`testSidebarHealthStripShowsMixedHealthStates`, `testSidebarHealthStripStaysAbsentWithoutHealthSnapshot`) を追加した。
- **Verification**:
  - `xcodebuild -project AgtmuxTerm.xcodeproj -scheme AgtmuxTerm -destination 'platform=macOS' -only-testing:AgtmuxTermUITests/AgtmuxTermUITests/testSidebarHealthStripShowsMixedHealthStates -only-testing:AgtmuxTermUITests/AgtmuxTermUITests/testSidebarHealthStripStaysAbsentWithoutHealthSnapshot test`
    - 初回 2 回は runner 初期化で `Timed out while enabling automation mode` により失敗。
    - XCUITest 権限付与後の rerun は PASS（`2 tests`, `0 failures`）。
  - `xcodebuild -project AgtmuxTerm.xcodeproj -scheme AgtmuxTerm -destination 'platform=macOS' build` は PASS。
- **Files**:
  - `Sources/AgtmuxTermCore/AccessibilityID.swift`
  - `Sources/AgtmuxTerm/SidebarView.swift`
  - `Tests/AgtmuxTermUITests/UITestHelpers.swift`
  - `Tests/AgtmuxTermUITests/*`
- **Acceptance Criteria**:
  - [x] health strip の stable accessibility contract が定義される
  - [x] degraded / unavailable surfacing の UI coverage が追加される
  - [x] health strip regression が build-only ではなく test で検知できる
  - [x] targeted XCUITest verification が automation mode 利用可能な環境で通る

### T-082 — A2 follow-up: `ui.health.v1` XPC client contract coverage 固定 (2026-03-06)
- **Status**: DONE
- **Priority**: P1
- **Description**:
  - `T-080` で direct path と app-side classification の coverage は入ったが、XPC client 側の `ui.health.v1` reply handling は dedicated test でまだ固定していなかった。
  - A2 closeout 前に、protocol-compatible reply contract に対して typed decode と structured unsupported payload preservation を確認する。
- **Planned**:
  - injected proxy 経由で `AgtmuxUIHealthV1` success payload を返す focused test を追加する。
  - `ui.health.v1` unsupported の structured envelope が XPC client で保持されることを固定する。
  - direct path と XPC path で classification contract がずれないことを確認する。
- **Implemented**:
  - `AgtmuxDaemonXPCClient` に test-only proxy injection を追加し、XPC protocol reply contract を focused test で直接叩けるようにした。
  - `AgtmuxDaemonXPCClientTests` を追加し、`ui.health.v1` success decode と structured unsupported payload preservation を固定した。
- **Verification**:
...
- **Files**:
  ...
  - `Tests/AgtmuxTermCoreTests/*`
- **Acceptance Criteria**:
  - [x] XPC 経由の `ui.health.v1` success decode が test で固定される
  - [x] XPC 経由の structured unsupported payload preservation が test で固定される
  - [x] direct/XPC で health classification contract が一致する

### T-083 — A2 follow-up: hosted XPC serialization-boundary で `ui.health.v1` coverage 固定 (2026-03-06)
- **Status**: DONE
- **Priority**: P1
- **Description**:
  - `T-082` で XPC client の reply contract coverage は固定したが、fake proxy を使わずに real XPC serialization boundary を跨ぐ coverage はまだ dedicated test で固定していなかった。
  - A2 closeout 前に、anonymous listener を使う hosted harness で success/error path を検証し、client-side proxy injection だけに依存しない証跡を追加する。
- **Planned**:
  - anonymous listener を使う hosted XPC harness を作る。
  - `ui.health.v1` success payload が real XPC serialization boundary 越しでも decode されることを固定する。
  - structured unsupported payload が real XPC serialization boundary 越しでも保持されることを固定する。
- **Implemented**:
  - `AgtmuxDaemonXPCClient` に `NSXPCListenerEndpoint` override を追加し、proxy injection を使わずに real XPC serialization boundary を跨げるようにした。
  - anonymous `NSXPCListener` を使う hosted integration test harness を追加し、service-side bridge から real `AgtmuxDaemonClient` を呼ぶ形で success/error path を固定した。
  - `AgtmuxTermIntegrationTests` target の hosted-unit-test wiring (`TEST_TARGET_NAME` / `TEST_HOST` / target dependency) を補正し、Xcode からも integration test を実行できるようにした。
- **Verification**:
...
- **Files**:
  ...
  - `AgtmuxTerm.xcodeproj/project.pbxproj`
- **Acceptance Criteria**:
  - [x] real XPC serialization boundary 越しで `ui.health.v1` success decode が test で固定される
  - [x] real XPC serialization boundary 越しで structured unsupported payload preservation が test で固定される
  - [x] service boundary を含む coverage が docs/progress に反映される

### T-084 — A2 follow-up: bundled `AgtmuxDaemonServiceEndpoint` 実経路の `ui.health.v1` coverage 固定 (2026-03-06)
- **Status**: DONE
- **Priority**: P1
- **Description**:
  - `T-083` で anonymous listener を使う hosted XPC serialization-boundary coverage は固定したが、actual bundled service endpoint (`AgtmuxDaemonServiceEndpoint`) 自体はまだ dedicated test で固定していない。
  - A2 closeout 前に、`AgtmuxDaemonXPCClient -> AgtmuxDaemonServiceEndpoint -> AgtmuxDaemonClient` の実経路を test で押さえ、service-side forwarding の取りこぼしを防ぐ。
- **Completed**:
  - actual `AgtmuxDaemonServiceEndpoint` 実装を `ServiceEndpoint.swift` に分離し、production の XPC service target と non-hosted test bundle の両方から同じ forwarding logic を共有できるようにした。
  - `AgtmuxTermCoreTests` に `AgtmuxDaemonXPCClient.swift` / `ServiceEndpoint.swift` / `AgtmuxDaemonXPCServiceBoundaryTests.swift` を追加し、app-hosted harness を経由しない focused endpoint-path coverage を固定した。
  - `AgtmuxDaemonServiceEndpointTests` で `ui.health.v1` success decode と structured unsupported payload preservation を actual service endpoint forwarding 越しに確認した。
- **Files**:
  - `Sources/AgtmuxDaemonService/main.swift`
...
- **Acceptance Criteria**:
  - [x] actual `AgtmuxDaemonServiceEndpoint` 越しで `ui.health.v1` success decode が test で固定される
  - [x] actual `AgtmuxDaemonServiceEndpoint` 越しで structured unsupported payload preservation が test で固定される
  - [x] service endpoint forwarding を含む coverage が docs/progress に反映される

### T-085 — V2 docs realignment: tmux-first cockpit baseline (2026-03-06)
- **Status**: DONE
- **Priority**: P0
- **Description**:
  - linked-session / Phase 3 workspace を main product truth として置き続けると、現行の V2 方向性と docs が矛盾したままになる。
  - foundation / spec / architecture / design / plan を、`tmux first`, `terminal stays terminal`, `Workbench`, `explicit companion surfaces` を前提に再整列する。
- **Planned**:
  - `docs/10_foundation.md` を tmux-first cockpit の stable intent に更新する。
  - `docs/20_spec.md` の `[MVP]` を real-session terminal tiles + browser/document companion surfaces + explicit restore errors ベースへ更新する。
  - `docs/30_architecture.md` / `docs/40_design.md` / `docs/50_plan.md` を V2 mainline に差し替え、旧 linked-session path は history としてのみ扱う。
  - `docs/70_progress.md` に product direction pivot を記録する。
- **Implemented**:
  - `docs/10_foundation.md` を tmux-first cockpit baseline に更新した。
  - `docs/20_spec.md` を V2 MVP baseline に更新し、real-session terminal tiles / Workbench / CLI bridge / explicit restore errors を mainline spec にした。
  - `docs/30_architecture.md` / `docs/40_design.md` / `docs/50_plan.md` を V2 mainline に差し替え、linked-session path を deprecated mainline として扱うよう更新した。
  - `docs/80_decisions/ADR-20260306-tmux-first-cockpit-v2.md` を追加し、mainline pivot を ADR として固定した。
  - `docs/90_index.md` を V2 doc set と current direction に合わせて更新した。
  - `docs/70_progress.md` に docs realignment phase を記録した。
- **Acceptance Criteria**:
  - [x] `docs/10_foundation.md` が V2 product intent と一致する
  - [x] `docs/20_spec.md` の `[MVP]` が V2 baseline と一致する
  - [x] `docs/30_architecture.md` が V2 mainline architecture と一致する
  - [x] `docs/40_design.md` が Workbench / CLI bridge / restore / remote semantics を mainline truth として持つ
  - [x] `docs/50_plan.md` / `docs/70_progress.md` が pivot を反映する

### T-086 — V2 docs consistency pass: design-lock integration (2026-03-06)
- **Status**: DONE
- **Priority**: P1
- **Description**:
  - `T-085` で mainline pivot は反映できたが、実装者が迷わないレベルまで design-lock の細部を main docs に揃える必要がある。
  - 特に `TargetRef`, OSC bridge, autosave/pinning, duplicate open, manual `Rebind`, directory tile の将来拡張を main docs に明文化する。
- **Implemented**:
  - `docs/30_architecture.md` に `TargetRef` の stable identity、terminal-scoped OSC bridge の boundary、unpinned companion context の drop、future directory surface の reserved extension を追記した。
  - `docs/40_design.md` に `TargetRef` の非推測ルール、reserved `agt reveal`, directory input の explicit rejection、future additive extension rule を追記した。
  - `docs/50_plan.md` に directory input rejection と directory surface 非MVPを明記した。
  - `docs/90_index.md` に docs realignment + design-lock integration を current tracking focus として反映した。
  - `docs/70_progress.md` に docs-only consistency pass を記録した。
- **Acceptance Criteria**:
  - [x] architecture/design が `TargetRef = local or configured remote key` を明示する
  - [x] CLI bridge が OSC first / explicit failure / directory input rejection を明示する
  - [x] future directory work が additive post-MVP extension として main docs に固定される
  - [x] tracking docs が consistency pass を記録する
