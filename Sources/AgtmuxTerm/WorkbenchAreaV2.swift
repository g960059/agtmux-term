...
enum WorkbenchTerminalPaneIdentity {
...
private struct WorkbenchTerminalTileViewV2: View {
    ...
    private var activePaneRuntimeContext: (
        ...
        return (
            desiredPaneRef: context.desiredPaneRef?.matches(sessionRef: sessionRef) == true
                ? WorkbenchTerminalPaneIdentity.normalized(context.desiredPaneRef)
                : nil,
            observedPaneRef: context.observedPaneRef?.matches(sessionRef: sessionRef) == true
                ? WorkbenchTerminalPaneIdentity.normalized(context.observedPaneRef)
                : nil,
    ...
    private var visiblePaneIdentity: String? {
        WorkbenchTerminalPaneIdentity.visiblePaneIdentity(for: activePaneRef)
    }
